...
-   가용성 영역을 지원하는 Azure 지역에 충분한 수의 사용 가능한 Dsv3 vCPU(vCPU 2개가 포함된 Standard_D2s_v3 VM 4개와 각각 vCPU 4개가 있는 Standard_D4s_v3 6개)가 있는 Microsoft Azure 구독
...
1.  **가용성 영역을 사용하여 DC가 2개인 새 AD 도메인 만들기** 블레이드에서 다음 설정을 지정하고 **검토 + 만들기**를 클릭한 다음 **구매**를 클릭하여 배포를 시작합니다.
...
1.  **SAP NetWeaver 3계층(관리 디스크)** 블레이드로 돌아와 다음 설정을 지정하고 **검토 + 만들기**를 클릭한 후에 **만들기**를 클릭하여 배포를 시작합니다.
...
### 작업 4: SOFS(스케일 아웃 파일 서버) 클러스터 배포
...
1.  **클러스터에 대한 고급 보안 설정**의 **사용 권한** 탭에서 **추가**를 클릭합니다.
...
1.  **장애 조치(failover) 클러스터 관리자** 콘솔에서 노드, 감시, 네트워크 설정을 비롯한 **az12003b-db-cl0** 클러스터 구성을 검토합니다. 클러스터에는 공유 스토리지가 없습니다.
